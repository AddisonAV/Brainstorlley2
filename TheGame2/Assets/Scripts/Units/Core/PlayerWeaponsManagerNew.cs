...
{
    [RequireComponent(typeof(PlayerInputHandlerNew))]
    public class PlayerWeaponsManagerNew : MonoBehaviour
    {
        ...
        PlayerInputHandlerNew m_InputHandler;
        PlayerCharacterControllerNew m_PlayerCharacterControllerNew;
        ...
        void Start()
        {
            ...
            m_InputHandler = GetComponent<PlayerInputHandlerNew>();
            DebugUtility.HandleErrorIfNullGetComponent<PlayerInputHandlerNew, PlayerWeaponsManagerNew>(m_InputHandler, this,
                gameObject);

            m_PlayerCharacterControllerNew = GetComponent<PlayerCharacterControllerNew>();
            DebugUtility.HandleErrorIfNullGetComponent<PlayerCharacterControllerNew, PlayerWeaponsManagerNew>(
                m_PlayerCharacterControllerNew, this, gameObject);
        ...
        {
            m_PlayerCharacterControllerNew.PlayerCamera.fieldOfView = fov;
        ...
        void UpdateWeaponAiming()
        {
            if (m_WeaponSwitchState == WeaponSwitchState.Up)
            {
                ...
                if (IsAiming && activeWeapon)
                {
                    ...
                    SetFov(Mathf.Lerp(m_PlayerCharacterControllerNew.PlayerCamera.fieldOfView,
                ...
                else
                {
                    ...
                    SetFov(Mathf.Lerp(m_PlayerCharacterControllerNew.PlayerCamera.fieldOfView, DefaultFov,
                        AimingAnimationSpeed * Time.deltaTime));
                }
            }
        }
        ...
        void UpdateWeaponBob()
        {
            if (Time.deltaTime > 0f)
            {
                Vector3 playerCharacterVelocity =
                    (m_PlayerCharacterControllerNew.transform.position - m_LastCharacterPosition) / Time.deltaTime;

                // calculate a smoothed weapon bob amount based on how close to our max grounded movement velocity we are
                float characterMovementFactor = 0f;
                if (m_PlayerCharacterControllerNew.IsGrounded)
                {
                    characterMovementFactor =
                        Mathf.Clamp01(playerCharacterVelocity.magnitude /
                                      (m_PlayerCharacterControllerNew.MaxSpeedOnGround *
                                       m_PlayerCharacterControllerNew.SprintSpeedModifier));
                }
                ...
                m_LastCharacterPosition = m_PlayerCharacterControllerNew.transform.position;
            }
        }
